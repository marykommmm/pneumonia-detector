...
function Navigation({ isLoggedIn, user, onLogout, onLogin }) {
  // 🚀 i18n ДЛЯ ПЕРЕМИКАННЯ МОВИ
  const { t, i18n } = useTranslation();
  const navigate = useNavigate();
  const location = useLocation();

  // Стейт тепер правильно оголошений ВСЕРЕДИНІ компонента 👍
  ...
  // 🔄 Автоматично закриваємо мобільний сайдбар, коли користувач клікає на будь-яке посилання і змінює сторінку
  useEffect(() => {
    setMobileMenuOpen(false);
  }, [location]);

  // Твій useEffect для теми оформлення
  useEffect(() => {
  ...
  return (
    <>
      ...
      {isLoggedIn ? (
        <>
          {/* Мобільна верхня панель (Header) для авторизованого користувача */}
          {/* Мобільна верхня панель (Header) для авторизованого користувача */}
          ...
          {/* Бічний сайдбар (контролюється класом .open через state) */}
          <aside className={`sidebar ${mobileMenuOpen ? "open" : ""}`}>
            <div className="sidebar-logo">
              ...
              {/* Кнопка "Х" для закриття сайдбару на мобільних пристроях */}
              <button
        ...
        /* 🌐 ГОСТЬОВИЙ РЕЖИМ: Верхня панель (Залишаємо інлайнові стилі, як у тебе) */
        <header
        ...
        >
          ...
          <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
            <div
              className="lang-switcher-guest"
              style={{ marginRight: "15px" }}
            >
              <button
                className={`lang-btn-guest ${i18n.language === "uk" ? "active" : ""}`}
                onClick={() => changeLanguage("uk")}
              >
                UA
              </button>
              <span className="lang-divider-guest">|</span>
              <button
                className={`lang-btn-guest ${i18n.language === "en" ? "active" : ""}`}
                onClick={() => changeLanguage("en")}
              >
                EN
              </button>
